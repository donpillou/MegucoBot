...
{
  // todo: save timeout in BotProtocol::Order, load order here to allow simulation of sessions that do already contain some data
}
...
void_t SimBroker::handleTrade(const DataProtocol::Trade& trade)
{
  ...
  for(List<Order>::Iterator i = openOrders.begin(), end = openOrders.end(), next; i != openOrders.end(); i = next)
  {
    next = i;
    ++next;
            ...
            ( order.type == BotProtocol::Order::sell && trade.price > order.price) )
    {
      ...
      botConnection.createSessionTransaction(transaction, transaction.entityId);
      ...
      botConnection.createSessionMarker(marker, marker.entityId);
      continue;
    }
...
bool_t SimBroker::buy(double price, double amount, timestamp_t timeout)
{
  ...
  order.price = price;
  order.fee = fee;
  botConnection.createSessionOrder(order, order.entityId);
...
uint_t SimBroker::getOpenBuyOrderCount() const
{
  ...
  for(List<Order>::Iterator i = openOrders.begin(), end = openOrders.end(); i != end; ++i)
  {
    const Order& order = *i;
    if(order.type == BotProtocol::Order::buy)
      ++openBuyOrders;
